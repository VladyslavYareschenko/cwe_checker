...
use kernel::analysis::graph;
use kernel::utils::binary::{BareMetalConfig, RuntimeMemoryImage};
use kernel::utils::log::print_all_messages;
use kernel::utils::{get_ghidra_plugin_path, read_config_file};
use kernel::AnalysisResults;
use kernel::{intermediate_representation::Project, utils::log::LogMessage};
...
#[no_mangle]
pub unsafe extern fn run_for_path(file_path: *const c_char) {
    println!("Hello world!");
    let raw = CStr::from_ptr(file_path).to_str().unwrap();
    let args = CmdlineArgs {
        binary: Some(raw.to_owned()),
        config: None,
        out: None,
        partial: None,
        json: false,
        quiet: false,
        statistics: false,
        bare_metal_config: None,
        module_versions: false,
        debug: false
    };

    run_with_ghidra(&args);
}
...
fn run_with_ghidra(args: &CmdlineArgs) {
    ...
    if args.debug {
        kernel::analysis::pointer_inference::run(
            &project,
            &runtime_memory_image,
            &control_flow_graph,
...
fn filter_modules_for_partial_run(
    modules: &mut Vec<&kernel::CweModule>,
    partial_param: &str,
) {
...
fn get_project_from_ghidra(
    ...
    let mut project_pcode: kernel::pcode::Project =
        serde_json::from_reader(std::io::BufReader::new(file)).unwrap();
    let mut log_messages = project_pcode.normalize();
    let project: Project = match kernel::utils::get_binary_base_address(binary) {
