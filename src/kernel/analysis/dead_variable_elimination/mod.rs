...
use crate::kernel::analysis::backward_interprocedural_fixpoint::create_computation;
use crate::kernel::analysis::graph::Node;
...
pub fn compute_alive_vars(project: &Project) -> HashMap<Tid, HashSet<Variable>> {
    let extern_subs = project
        ...
        .collect();
    let mut graph = crate::kernel::analysis::graph::get_program_cfg(&project.program, extern_subs);
    graph.reverse();
